...
int main(int argc, char* argv[]) {
  ...
  // open userio module
    ...
    if (evt2Code == WIIMOTE_BTN_UP) { userio_ledSet(pMemBase, 0, evt2Value); }
    else if (evt2Code == WIIMOTE_BTN_DOWN) { userio_ledSet(pMemBase, 1, evt2Value); }
    else if (evt2Code == WIIMOTE_BTN_LEFT) { userio_ledSet(pMemBase, 2, evt2Value); }
    else if (evt2Code == WIIMOTE_BTN_RIGHT) { userio_ledSet(pMemBase, 3, evt2Value); }
    else if (evt2Code == WIIMOTE_BTN_A) { userio_ledSet(pMemBase, 4, evt2Value); }
    else if (evt2Code == WIIMOTE_BTN_B) { userio_ledSet(pMemBase, 5, evt2Value); }
    else if (evt2Code == WIIMOTE_BTN_1) { userio_ledSet(pMemBase, 6, evt2Value); }
    else if (evt2Code == WIIMOTE_BTN_2) { userio_ledSet(pMemBase, 7, evt2Value); }
    else if (evt2Code == WIIMOTE_BTN_HOME) { stayInLoop = 0; }
	}
